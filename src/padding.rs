...
pub fn _check_set_padding<T: PaddingGenerator>(p: &T, pt: PaddingTuple) {
...
pub fn _check_remove_padding<T: PaddingGenerator>(p: &T, pt: PaddingTuple) {
	let (raw_text, b, padded_text) = pt;
...
pub fn _check_padding<T: PaddingGenerator>(p: &T, pt: PaddingTuple) {
	_check_set_padding(p ,pt);
	_check_remove_padding(p, pt);
}